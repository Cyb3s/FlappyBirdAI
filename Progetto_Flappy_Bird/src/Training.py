...
def eval_genomes(genomes, config):
    ...
    while running and len(players) > 0:
        ...
        for x, player in enumerate(players):  # give each bird a fitness of 0.1 for each frame it stays alive
            ...
            output = nets[players.index(player)].activate(inputs)
        ...
        for player in players:
        #player die or reach training score limit
            if player.rect.y > 836-215 or pygame.sprite.spritecollide(player, pipe_list, False) or player.rect.y < 0 or score > 10:
...
#function of genomes' configuration
def run(config_file):
    ...
    # Run for up to 4 generations.
    winner = p.run(eval_genomes, 4)

    # show final stats
    print('\nBest genome:\n{!s}'.format(winner))
    
    visualize.draw_net(config,winner,filename="Best_genome")
